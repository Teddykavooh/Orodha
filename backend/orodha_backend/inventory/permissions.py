...
class AdminOrReadOnly(permissions.BasePermission):
    ...
    Non-safe methods require WHOLESALER_ADMIN.
    """

    def has_permission(self, request, view):
        if request.method in permissions.SAFE_METHODS:
            return bool(request.user and request.user.is_authenticated)
        return bool(request.user and _is_wholesaler_admin(request.user))
...
class CanDeleteInventory(permissions.BasePermission):
    ...
    Only WHOLESALER_ADMIN may create/update/delete inventory resources.
    """
