...
class ReportingDashboardView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 2. 📊 CASE A: STREAM OUT STANDARD EXCEL-COMPATIBLE CSV
        if export_format == "csv":
            response = HttpResponse(content_type="text/csv")
            response["Content-Disposition"] = 'attachment; filename="orodha_sales_report.csv"'
        ...
        # 3. 📄 CASE B: GENERATE AND STREAM CLEAN CORPORATE PDF
        elif export_format == "pdf":
            response = HttpResponse(content_type="application/pdf")
            response["Content-Disposition"] = 'attachment; filename="orodha_sales_report.pdf"'
            ...
            story.append(Paragraph("Orodha Ledger Systems - Sales Report", title_style))
            story.append(Paragraph(f"Generated By Account: {user.username} | Records Compiled: {queryset.count()} logs found.", meta_style))
            story.append(Spacer(1, 10))
            
            table_data = [["Receipt ID", "Timestamp", "Product Description", "Agent", "Price (KES)"]]
            for log in queryset:
                table_data.append([
                    f"#REC-{log.id}",
                    log.sold_at.strftime("%Y-%m-%d %H:%M"),
            ...
            # 🔑 THE FIX: Append the completed table to the story flow array and execute the document build stream
            story.append(report_table)
            doc.build(story)
            return response

        # 4. 🧠 CASE C: DEFAULT APP SUMMARY METRICS (JSON API CALL FALLBACK)
